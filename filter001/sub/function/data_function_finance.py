...
def get_finance_info(ticker,max_price,min_price,min_volume, dividend_status, target_yield):
    # Ticker オブジェクトを作成
    ...
    if volume_day10 is None or volume_day10 < min_volume:
        return None
    
    dividend_last = info.get("trailingAnnualDividendRate", None)
    if dividend_last is None or dividend_last < dividend_status +1:
        return None
    
    this_year_yield =info.get("dividendYield", None)
    if this_year_yield is None or this_year_yield < target_yield*0.01:
        return None
    ...
    new_row = {
        ...
        #この8つでない
        #"買い注文数": info.get("bidSize", None),
        #"売り注文数": info.get("askSize", None),
        #"24時間取引量": info.get("volume24Hr", None),
        #"空売株数": info.get("sharesShort", None),
        #"予想配当額": info.get("forwardAnnualDividendRate", None),
        #"予想配当利回り": info.get("forwardAnnualDividendYield", None),
        #"空売比率": info.get("shortRatio", None),
        #"浮動株空売割合": info.get("shortPercentOfFloat", None),
        
        #"決算日差":dif_earnings_date,
        #"配当日差":dif_ex_dividend_data,
        #"分割日差":dif_last_split_date
    }
    #取引量
