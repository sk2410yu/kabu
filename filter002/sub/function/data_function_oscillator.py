...
# 短期RSIを計算する関数
def calculate_rsiShort(df, column='Close', window=9):
...
#短期RCI 一定期間の順位を表す　買うは底値で売り少し待って警戒
# RCI (Rank Correlation Index) を計算し、DataFrameに追加する汎用関数
import pandas as pd
...
# 短期RCIを計算する関数
def calculate_rciShort(df, column='Close', window=9):
    return calculate_rci(df, column, window)

# 中期RCIを計算する関数
def calculate_rciMid(df, column='Close', window=14):
    return calculate_rci(df, column, window)
...
def get_calculate_oscillator(df):
    # オシレータ
    df = calculate_rsiShort(df)
    df = calculate_rsiLong(df)
    df = calculate_stochastic(df)
    df = calculate_psychological_line(df)
    df = calculate_rciShort(df)
    df = calculate_rciMid(df)
    df = calculate_rciLong(df)
    df = calculate_ma_deviation(df)
    # if でここのリストから排除RSI,MACD　for で 辞書型で関数実行　return変更で　　列で合わせる　sum_df　としてfor 後　リターンdf　
